...
export default () => {
  ...
  const submit = async (values) => {
    ...
    setLoadSubmit(true);
    try {
      const resWithdrawSave = await API.withdraw.post(data);
      resWithdrawSave && resWithdrawSave?.isSuccess === true
        ? onSuccess(resWithdrawSave)
        : onFailed(resWithdrawSave);
    } catch (ex) {
      setLoadSubmit(false);
      notification["error"]({
        message: "Error",
